...
export function AllCategoryPage() {
  ...
  const handleEditCategory = async (e) => {
    ...
    try {
      await axios.put(
        `${BACKEND_URL}update-category/${editItem.id}`,
        {
  ...
  const handleAddSubcategory = async (e) => {
    ...
    try {
      await axios.post(
        `${BACKEND_URL}add-subcategory/${selectedCategory.id}`,
        {
                      ...
                      <div
                        ...
                        className={`${
                          ...
                          selectedCategory.id === category.id
                            ? "bg-red-50"
                            : ""
                        } grid grid-cols-1 sm:grid-cols-12 py-3 px-2 sm:px-0 border-b hover:bg-gray-50 cursor-pointer`}
        ...
        <div className="w-full lg:w-1/2 bg-white rounded-lg border shadow-sm p-4">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2">
            <h3 className="text-lg font-medium">
              {selectedCategory
                ? `Subcategories for ${selectedCategory.categories}`
                : "Subcategories"}
            </h3>
            <button
              className={`bg-red-500 hover:bg-red-600 text-white py-2 px-4 rounded-md flex items-center justify-center w-full sm:w-auto ${
                ...
                selectedCategory.subCategories.length > 0 ? (
                    ...
                    <div className="bg-white divide-y divide-gray-200">
                      {selectedCategory.subCategories.map(
                        (subcategory, idx) => (
                          <div
                            key={idx}
                            className="grid grid-cols-1 sm:grid-cols-8 py-3 px-2 sm:px-0 border-b"
                          >
                            <div className="col-span-5 px-2 sm:px-6 py-1 sm:py-0 whitespace-nowrap">
                              <div className="text-sm font-medium text-gray-900">
                                {subcategory}
                  ...
                  <div className="text-center py-10">
                    ...
                    <p className="text-sm text-gray-400 mt-2">
                      Add a subcategory to {selectedCategory.categories}
                    </p>
                  </div>
                )
              ) : (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-sm">
            <h3 className="text-lg font-medium mb-4">
              Add Subcategory to {selectedCategory.categories}
            </h3>
...
export function CategoryPage() {
  ...
  const handleDeleteSection = async (sectionId) => {
    const confirmDelete = window.confirm(
      "Are you sure you want to delete this section? This will also delete all subsections within it."
    );
    if (!confirmDelete) return;
      ...
      setSections((currentSections) => {
        return currentSections.map((section) => {
          if (section.id === selectedSection.id) {
            return {
              ...section,
              subCategory: section.subCategory.filter(
  ...
  const handleEditSection = async (e) => {
    ...
    try {
      await axios.put(
        `${BACKEND_URL}update-section/${editItem.data.id}`,
        {
    ...
    <div className="p-4">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="w-full lg:w-1/2 bg-white rounded-lg border shadow-sm p-4">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2">
            <h3 className="text-lg font-medium">
              {selectedSection
                ? `Subsections for ${selectedSection.category}`
                : "Subsections"}
            </h3>
            <button
              className={`bg-red-500 hover:bg-red-600 text-white py-2 px-4 rounded-md flex items-center justify-center w-full sm:w-auto ${
          ...
          <div className="overflow-x-auto">
            <div className="overflow-y-auto max-h-[calc(100vh-300px)]">
              {isLoading && selectedSection ? (
                ...
                selectedSection.subCategory.length > 0 ? (
                  <div className="min-w-full">
                    <div className="bg-gray-50 py-3 hidden sm:grid grid-cols-8">
                      <div className="col-span-5 px-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        Name
                  ...
                  <div className="text-center py-10">
                    ...
                    <p className="text-sm text-gray-400 mt-2">
                      Add a subsection to {selectedSection.category}
                    </p>
                  </div>
                )
              ) : (
      ...
      {showAddSubsection && selectedSection && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-sm">
            <h3 className="text-lg font-medium mb-4">
              Add Subsection to {selectedSection.category}
            </h3>
